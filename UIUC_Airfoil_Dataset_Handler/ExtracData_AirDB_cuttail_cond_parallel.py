...
if __name__ == "__main__":
    ...
    # 
    XC = np.reshape(xc_star, (Nfoil,Ncon,N)) # Nfoil x C x N 
    YC = np.reshape(yc_star, (Nfoil,Ncon,N)) # 
    CC = np.tile(con_star, (Nfoil,1,N)) # 
    DC = np.reshape(dc_star, (Nfoil,Ncon,N)) # Nfoil x c x N     
    ...
    for i in range(0,2):
        for snap in range(Ncon):
            ...
            np.savetxt(write_path+"Case_flo_Foil="+str(i*Ncon+snap)+"_parallel.dat", p3d_result, delimiter=" ", header=Tecplot_header_out, comments=' ')

...
def predict_drag_lift(t_sur, x_sur, y_sur, usur_star, vsur_star, psur_star):
...
if __name__ == "__main__":
    ...
    print(shp)
    ###
    t_star = np.arange(Ntime)[:,None]*inc_time*dt # T(=1) x 1
    T = t_star.shape[0]
    xc_star = array_data1[:,0] # NT x 1
    yc_star = array_data1[:,1] # NT x 1
    NT = xc_star.shape[0]
    uc_star = array_data1[:,2] #fix 4  3
    vc_star = array_data1[:,3] #fix 5  4
    pc_star = array_data1[:,4] #fix 7  5
    ...
    F_L, F_D = predict_drag_lift(t_sur, x_sur, y_sur, u_sur, v_sur, p_sur)
    result = np.hstack((F_L[:,None], F_D[:,None]))
    np.savetxt(sim_data_path+"PINN_CLCD_cuttail.dat", result, delimiter=" ", header="variables = CL, CD")
    #np.savetxt("./FOM_CLCD_RANS3.dat", result, delimiter=" ", header="variables = CL, CD")

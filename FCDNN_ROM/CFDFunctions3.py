...
def Euler_uIncomp_POD(a, phi, mean_data, t, x, y, Pec, Gamma):
    ...
    for i in range(T):
        ...
        if i == 0:
            ...
            #UC_pred = tf.concat([UC_pred, temp], axis=0)
            D_pred = tf.transpose(tf.concat([D_pred, temp[:,0]], axis=0))
            U_pred = tf.transpose(tf.concat([U_pred, temp[:,1]], axis=0))
            V_pred = tf.transpose(tf.concat([V_pred, temp[:,2]], axis=0))
            P_pred = tf.transpose(tf.concat([P_pred, temp[:,3]], axis=0))
    d = tf.reshape(D_pred, [-1]) #U_pred[:,0]
    u = tf.reshape(U_pred, [-1])[:,None]  #U_pred[:,1][:,None]
    v = tf.reshape(V_pred, [-1])[:,None]  #U_pred[:,2][:,None]
    p = tf.reshape(P_pred, [-1])[:,None]  #U_pred[:,3][:,None]
    ...
    #e4 = d*E_t + p_x*u + p*u_x + p_y*v + p*v_y + d*u*E_x + d*v*E_y
    #print(e1,e2,e3,e4)

    return e1, e2, e3

def Euler_uIncomp_POD2(a, phi, mean_data, t, x, y, Pec, Gamma):
    ...
    mean_data_tf = tf.constant(mean_data, tf.float64)
    U_pod_pred = tf.add(tf.transpose(tf.matmul(a, tf.transpose(phi_tf))), 
                        tf.tile(mean_data_tf, tf.constant([1,T], tf.int32)))
    for i in range(T):
        temp = tf.reshape(U_pod_pred[:,i], [-1, noConcernVar])
        #print(T, temp) 
        if i == 0:
            ...
            V_pred = tf.transpose(tf.concat([V_pred, temp[:,2]], axis=0))
            P_pred = tf.transpose(tf.concat([P_pred, temp[:,3]], axis=0))
    d = tf.reshape(D_pred, [-1])[:,None]  #U_pred[:,0]
    u = tf.reshape(U_pred, [-1])[:,None]  #U_pred[:,1][:,None]
    v = tf.reshape(V_pred, [-1])[:,None]  #U_pred[:,2][:,None]
    p = tf.reshape(P_pred, [-1])[:,None]  #U_pred[:,3][:,None]

    Y = tf.concat([u, v, p, a], 1)
    print(x, U_pred)
    Y_t = fwd_gradients(Y, t)
    Y_x = fwd_gradients(Y, x)
    Y_y = fwd_gradients(Y, y)
    ...
    Nox = -u*u_x - v*u_y - p_x
    Noy = -u*v_x - v*v_y - p_y
    #Nox_phi[i] = tf.tensordot(Nox,phi_tf[:,i])
    #Noy_phi[i] = tf.tensordot(Noy,phi_tf[:,i])
...
def Euler_uIncomp_2D(u, v, p, t, x, y, Pec, Gamma):
#def Euler_uIncomp_2D(u, v, t, x, y, Pec, Gamma):
...
def NS_Comp_2D(c, u, v, p, t, x, y, Pec, Rey):

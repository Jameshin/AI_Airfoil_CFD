...
if __name__ == "__main__":
    ...
    ######################################################################
        ...
        model.train(total_time = 10, learning_rate=1e-2)  # train until total_time (h)
        ...
        for i in range(20):
            ...
            np.savetxt(res_data_path+"Case_flo8_RANS_NN_cuttail2_t="+str(initial_time+i*inc_time+2)+".dat", p3d_result, delimiter=" ", header="variables = X, Y, c, u, v, p \n zone i="+str(zone1_i-2*cuttail)+" j="+str(glayer)+" ", comments=' ')
            # Error
